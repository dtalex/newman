...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        let certFile = _.has(options, 'sslCertInfo') ? options.sslCertInfo : undefined
        
        let certInfo = undefined;
        if (certFile ){
            let  contents = fs.readFileSync(certFile, 'utf8');
            certInfo = JSON.parse(contents);
        }
        else{
            certInfo = undefined;
        }
        ...
        runner.run(options.collection, {
            ...
            certificates: certInfo && new sdk.CertificateList({}, certInfo)
        }, function (err, run) {
